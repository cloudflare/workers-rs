...
pub(crate) fn header_map_from_web_sys_headers(
    ...
    for res in from_headers.entries().unwrap().into_iter() {
...
pub(crate) fn web_sys_headers_from_header_map(headers: &HeaderMap) -> web_sys::Headers {
    let output = web_sys::Headers::new().unwrap();
    for (key, value) in headers.iter() {
        output
            .append(key.as_str(), std::str::from_utf8(value.as_bytes()).unwrap())
            .unwrap();
    }
    output
}
