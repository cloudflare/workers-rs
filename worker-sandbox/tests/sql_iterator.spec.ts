...
describe("sql iterator durable object", () => {
  test("next() iterator returns typed results", async () => {
    const resp = await mf.dispatchFetch("http://fake.host/sql-iterator/test/next");
    ...
    expect(text).toContain("Product 1: Laptop - $999.99 (in stock: true)");
    expect(text).toContain("Product 2: Mouse - $29.99 (in stock: true)");
    expect(text).toContain("Product 3: Keyboard - $79.99 (in stock: false)");
    expect(text).toContain("Product 4: Monitor - $299.99 (in stock: true)");
    expect(text).toContain("Product 5: Headphones - $149.99 (in stock: false)");
  });

  test("raw() iterator returns column names and raw values", async () => {
    const resp = await mf.dispatchFetch("http://fake.host/sql-iterator/test/raw");
    expect(resp.status).toBe(200);
    ...
    expect(text).toContain('String("Laptop")');
    expect(text).toContain('String("Mouse")');
    expect(text).toContain('String("Keyboard")');
    expect(text).toContain('String("Monitor")');
    expect(text).toContain('String("Headphones")');
    
    // Check for different data types in raw format
    expect(text).toContain("Integer("); // For IDs and in_stock
    expect(text).toContain("Float("); // For prices
    expect(text).toContain("String("); // For names
  });

  test("different object instances have independent data", async () => {
    // Test first instance
    const resp1 = await mf.dispatchFetch("http://fake.host/sql-iterator/instance1/next");
    expect(resp1.status).toBe(200);
    const text1 = await resp1.text();
    expect(text1).toContain("Product 1: Laptop");
    
    // Test second instance - should have same seeded data
    const resp2 = await mf.dispatchFetch("http://fake.host/sql-iterator/instance2/next");
  ...
  test("iterator handles empty results gracefully", async () => {
    ...
    const resp = await mf.dispatchFetch("http://fake.host/sql-iterator/empty-test/raw");
    expect(resp.status).toBe(200);
  ...
  test("root endpoint returns help message", async () => {
    const resp = await mf.dispatchFetch("http://fake.host/sql-iterator/test");
    expect(resp.status).toBe(200);
  ...
  test("invalid endpoint path returns help message", async () => {
    const resp = await mf.dispatchFetch("http://fake.host/sql-iterator/test/invalid");
    expect(resp.status).toBe(200);
  ...
  test("data consistency between next() and raw() methods", async () => {
    ...
    const nextResp = await mf.dispatchFetch("http://fake.host/sql-iterator/consistency-test/next");
    expect(nextResp.status).toBe(200);
    const nextText = await nextResp.text();
    
    // Get data from raw() method  
    const rawResp = await mf.dispatchFetch("http://fake.host/sql-iterator/consistency-test/raw");
